...
const blank = { customer_id: '', amount: '', paid: '0', due_date: '', status: 'open', work_description: '' }
...
export default function InvoicesPage() {
  ...
  const [descriptionInvoice, setDescriptionInvoice] = useState(null)
  ...
  function editDescription(invoice) {
    const job = data.jobs.find(row => row.id === invoice.job_id)
    const contract = data.contracts.find(row => row.id === (invoice.contract_id || job?.contract_id))
    setDescriptionInvoice(invoice); setDescription(invoice.work_description || contract?.scope_of_work || ''); setDescriptionError('')
  }
  async function saveDescription(event) {
    event.preventDefault(); setBusyId(descriptionInvoice.id); setDescriptionError('')
    try {
      await workspace.updateAndWait('invoices', descriptionInvoice.id, { work_description: description.trim() })
      setMessage(`Work description saved for ${descriptionInvoice.number}.`); setDescriptionInvoice(null)
    } catch (error) { setDescriptionError(error.message) } finally { setBusyId('') }
  }
  ...
  return <section>
    ...
    <div className="table-wrap"><table><thead><tr><th>Invoice</th><th>Customer</th><th>Amount</th><th>Paid</th><th>Balance</th><th>Status</th><th>Actions</th></tr></thead><tbody>{data.invoices.map((invoice) => { const balance = Math.max(Number(invoice.amount) - Number(invoice.paid), 0); const disabled = busyId === invoice.id || invoice.status === 'void'; return <tr key={invoice.id}><td><strong>{invoice.number}</strong>{invoice.stripe_status ? <small className="table-subline">Stripe: {invoice.stripe_status.replaceAll('_', ' ')}</small> : null}</td><td>{customer(invoice.customer_id)?.full_name}</td><td>{money(invoice.amount)}</td><td>{money(invoice.paid)}</td><td><strong>{money(balance)}</strong></td><td><StatusBadge value={invoice.status} /></td><td><div className="invoice-actions"><button className="table-button" disabled={Boolean(busyId)} onClick={() => editDescription(invoice)}>Edit description</button><button className="table-button" disabled={disabled || balance <= 0} onClick={() => makeCardLink(invoice)}>{busyId === invoice.id ? 'Working…' : 'Copy card link'}</button>{invoice.receipt_token ? <><button className="table-button" onClick={() => shareReceipt(invoice)}>Share receipt</button><button className="table-button" onClick={() => emailReceipt(invoice)}>Email receipt</button><a className="table-button" href={`/receipt/${invoice.receipt_token}`} target="_blank" rel="noreferrer">View / PDF</a></> : null}<button className="table-button" disabled={disabled || balance <= 0} onClick={() => recordManualPayment(invoice)}>Record payment</button><button className="table-button danger" onClick={() => removeOrVoid(invoice)}>{Number(invoice.paid || 0) > 0 || invoice.stripe_checkout_session_id ? 'Void' : 'Delete'}</button></div></td></tr> })}</tbody></table></div>
    <Modal title="Work description" open={Boolean(descriptionInvoice)} onClose={() => { if (!busyId) setDescriptionInvoice(null) }}><form className="form-grid" onSubmit={saveDescription}><p className="wide">Shown on the customer invoice and printed receipt.</p><label className="wide">Description of work<textarea rows="6" value={description} onChange={event => setDescription(event.target.value)} maxLength={20000} /></label>{descriptionError && <p className="wide" role="alert">{descriptionError}</p>}<div className="form-actions wide"><button type="button" className="button secondary" disabled={Boolean(busyId)} onClick={() => setDescriptionInvoice(null)}>Cancel</button><button className="button primary" disabled={Boolean(busyId)}>{busyId ? 'Saving…' : 'Save description'}</button></div></form></Modal>
    <Modal title="New invoice" open={open} onClose={() => setOpen(false)}><form className="form-grid" onSubmit={save}><label className="wide">Customer<select value={form.customer_id} onChange={(event) => setForm({ ...form, customer_id: event.target.value })} required><option value="">Select…</option>{data.customers.map((item) => <option key={item.id} value={item.id}>{item.full_name}</option>)}</select></label><label className="wide">Description of work<textarea rows="4" value={form.work_description} onChange={event => setForm({ ...form, work_description: event.target.value })} maxLength={20000} placeholder="Describe the services included on this invoice" /></label><label>Invoice amount<input min="0.01" step=".01" type="number" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} required /></label><label>Already paid outside Stripe<input min="0" step=".01" type="number" value={form.paid} onChange={(event) => setForm({ ...form, paid: event.target.value })} /></label><label>Due date<input type="date" value={form.due_date} onChange={(event) => setForm({ ...form, due_date: event.target.value })} required /></label><div className="form-actions wide"><button type="button" className="button secondary" onClick={() => setOpen(false)}>Cancel</button><button className="button primary">Create invoice</button></div></form></Modal>
  </section>
}
