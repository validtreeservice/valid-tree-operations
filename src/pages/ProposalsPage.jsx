...
export default function ProposalsPage() {
  ...
  return <section className="proposals-workspace">
    ...
    {!form ? <>
      ...
      <div className="proposal-editor-actions"><div><button className="button secondary" onClick={back} disabled={busy}>← All proposals</button><span className={'proposal-status ' + proposalStatus(form)}>{proposalStatus(form)}</span>{dirty && <span>Unsaved changes</span>}</div>
        <div className="proposal-actions"><button className="button secondary" disabled={busy} onClick={() => showPreview()}>Preview</button><button className="button secondary" disabled={busy} onClick={() => showPreview(true)}>Generate PDF</button>
          {!locked && !form.deleted_at ? <><button className="button secondary" disabled={busy} onClick={() => run(save)}>Save Draft</button><button className="button primary" disabled={busy} onClick={() => setConfirmAction('send')}>Send Proposal</button>{form.number && <><button className="button secondary" disabled={busy || dirty} onClick={() => setConfirmAction('number')}>Proposal number</button></>}</> : !form.deleted_at && <><button className="button secondary" disabled={busy} onClick={duplicate}>Create revision</button>{form.share_token && <button className="button primary" onClick={() => setShare(true)}>Customer link</button>}</>}
          {form.number && !form.deleted_at && <button className="button secondary danger" disabled={busy || dirty} onClick={() => setConfirmAction('delete')}>Remove proposal</button>}
        </div>
      </div>
      {form.deleted_at && <div className="proposal-locked"><h2>Proposal in Trash</h2><p>The original proposal and any acceptance are preserved and its number will not be reused.</p><button className="button primary" disabled={busy} onClick={() => run(async () => { setForm(await api.trash(form, true)); setNotice('Proposal restored.'); setShowTrash(false) })}>Restore proposal</button></div>}
      ...
      {locked && !form.deleted_at && <div className="proposal-locked"><strong>{form.status === 'accepted' ? 'Accepted proposal - document locked' : 'Issued proposal - document locked'}</strong><p>{form.status === 'accepted' ? 'The signed version is preserved. Duplicate it to prepare a different offer.' : 'To make changes, withdraw this version. Its existing customer link will stop working.'}</p>
    ...
    <Modal title="Confirm proposal action" open={Boolean(confirmAction)} onClose={() => !busy && setConfirmAction('')}>
      ...
      {confirmAction === 'delete' && <><p>Move {form?.number} to Trash? It will leave the active list and its customer link will be unavailable while in Trash. The proposal, any recorded acceptance and its number are preserved. This does not cancel an agreement or delete linked jobs or invoices. You can restore it from Trash / restore.</p><button className="button primary" disabled={busy} onClick={() => run(async () => { await api.trash(form); setConfirmAction(''); ++latestOpen.current; setForm(null); setDirty(false); setParams({}); setNotice('Proposal moved to Trash. You can restore it at any time.') })}>Move proposal to Trash</button></>}
