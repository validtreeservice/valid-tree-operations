...
import { getContractTypeDefinition } from '../lib/contractTerms'
...
export default function PublicEstimatePage() {
      ...
      if (!supabase || String(token).startsWith('demo-')) {
        ...
        if (!contract) {
          ...
          contract = await workspace.addAndWait('contracts', {
            ...
            contract_number: nextDocumentNumber(workspace.data.contracts, definition.numberPrefix, { field: 'contract_number', floor: definition.key === 'tree_service' ? 38 : 0 }),
            contract_type: definition.key,
            title: definition.defaultTitle,
            scope_of_work: estimate.scope || `${definition.label} work described in the accepted estimate.`,
  ...
  const service = getContractTypeDefinition(estimate.service_type)
  return (
    <main className="public-document-page">
      <article className="public-document">
        ...
        <p className="eyebrow">Professional {service.label.toLowerCase()} estimate</p>
        <h1>{estimate.title || `${service.label} Estimate`}</h1>
        ...
        <h2>Scope of work</h2>
        <div className="public-scope">{String(estimate.scope || `${service.label} work as discussed.`).split('\n').map((line, index) => <p key={index}>{line}</p>)}</div>
