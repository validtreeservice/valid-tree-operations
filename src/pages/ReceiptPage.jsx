...
import { receiptPaymentState } from '../lib/receipt'
...
export default function ReceiptPage() {
  ...
  const { balance, paid, fullyPaid, label } = receiptPaymentState(invoice)
  const description = invoice.work_description?.trim()
  return <main className="receipt-page">
    ...
    <article className="receipt-document">
      ...
      <section className="receipt-title"><div><span>{paid ? 'Payment receipt' : 'Invoice'}</span><h2>{invoice.number}</h2></div><strong className={`receipt-stamp ${invoice.status === 'void' ? 'void' : ''}`}>{label}</strong></section>
      {fullyPaid && <div className="receipt-paid-banner"><strong>PAID IN FULL</strong><span>Payment received &middot; Balance due {money(0)}</span></div>}
      ...
      <section className="receipt-work"><h3>Work description</h3><p>{description || "No work description has been added. Please contact the office for details."}</p></section>
