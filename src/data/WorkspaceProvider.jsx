...
export function WorkspaceProvider({ children }) {
  ...
  const mutate = useCallback(async (action, collection, idOrItem, patch) => {
    ...
    if (action === 'add') {
      ...
      if (!isDemo && supabase && user) {
        ...
        // Invalidate any refresh that began after this save started but before
        // Supabase finished committing it. Otherwise that older response can
        // arrive after the local insert and make the new job disappear.
        mutationRevision.current += 1
        setData((current) => ({
          ...current,
  ...
  }), [data, loading, syncError, loadLive, isDemo, mutate])
